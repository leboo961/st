...
/*DynamicArray searchString =  UTF8_ARRAY; ffff */
...
/*void displayString(DynamicArray const *str, Glyph *g, int yPos) { ffff */
...
/// Default state if no operation is performed. ffff
...
tmoveto(int x, int y)
{
	...
	// Set the last position in order to restore after normal mode exits.
        onMove();
	/*stateNormalMode.initialPosition.x = term.c.x;  to delete if copmpiles ffff*/
